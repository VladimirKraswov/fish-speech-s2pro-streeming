...
FISH_CODES_QUEUE_MAXSIZE="${FISH_CODES_QUEUE_MAXSIZE:-4}"
FISH_AUDIO_QUEUE_MAXSIZE="${FISH_AUDIO_QUEUE_MAXSIZE:-4}"

# Для session mode приоритет — минимальный TTFA, а не максимальный throughput.
FISH_STREAM_MAX_AHEAD_CHUNKS="${FISH_STREAM_MAX_AHEAD_CHUNKS:-1}"
FISH_STREAM_CHUNK_SIZE="${FISH_STREAM_CHUNK_SIZE:-1}"
FISH_INITIAL_STREAM_CHUNK_SIZE="${FISH_INITIAL_STREAM_CHUNK_SIZE:-1}"
FISH_STREAM_START_BUFFER_CHUNKS="${FISH_STREAM_START_BUFFER_CHUNKS:-1}"

FISH_DAC_MAX_CODES_PER_STEP="${FISH_DAC_MAX_CODES_PER_STEP:-4}"
FISH_DAC_MIN_FREE_GB="${FISH_DAC_MIN_FREE_GB:-4.0}"
FISH_DAC_RESUME_FREE_GB="${FISH_DAC_RESUME_FREE_GB:-6.0}"
FISH_DAC_CRITICAL_FREE_GB="${FISH_DAC_CRITICAL_FREE_GB:-2.5}"
FISH_STREAM_EMPTY_CACHE="${FISH_STREAM_EMPTY_CACHE:-0}"
FISH_LLM_CRITICAL_FREE_GB="${FISH_LLM_CRITICAL_FREE_GB:-3.0}"
FISH_SESSION_IDLE_MIN_FREE_GB="${FISH_SESSION_IDLE_MIN_FREE_GB:-8.0}"
FISH_SESSION_IDLE_FORCE_HEAVY_BELOW_GB="${FISH_SESSION_IDLE_FORCE_HEAVY_BELOW_GB:-5.0}"
FISH_LIGHT_CUDA_CLEANUP_ON_SESSION_IDLE="${FISH_LIGHT_CUDA_CLEANUP_ON_SESSION_IDLE:-1}"
FISH_SESSION_CLEANUP_ON_CLOSE="${FISH_SESSION_CLEANUP_ON_CLOSE:-1}"
FISH_WARMUP_STREAMING="${FISH_WARMUP_STREAMING:-1}"
FISH_STREAMING_EAGER_DECODE="${FISH_STREAMING_EAGER_DECODE:-1}"
...
start_model() {
  ...
  local docker_args=(
    ...
    -e "FISH_CODES_QUEUE_MAXSIZE=$FISH_CODES_QUEUE_MAXSIZE"
    -e "FISH_AUDIO_QUEUE_MAXSIZE=$FISH_AUDIO_QUEUE_MAXSIZE"
    -e "FISH_STREAM_MAX_AHEAD_CHUNKS=$FISH_STREAM_MAX_AHEAD_CHUNKS"
    -e "FISH_STREAM_CHUNK_SIZE=$FISH_STREAM_CHUNK_SIZE"
    -e "FISH_INITIAL_STREAM_CHUNK_SIZE=$FISH_INITIAL_STREAM_CHUNK_SIZE"
    -e "FISH_STREAM_START_BUFFER_CHUNKS=$FISH_STREAM_START_BUFFER_CHUNKS"
    -e "FISH_DAC_MAX_CODES_PER_STEP=$FISH_DAC_MAX_CODES_PER_STEP"
    -e "FISH_DAC_MIN_FREE_GB=$FISH_DAC_MIN_FREE_GB"
    -e "FISH_DAC_RESUME_FREE_GB=$FISH_DAC_RESUME_FREE_GB"
    -e "FISH_DAC_CRITICAL_FREE_GB=$FISH_DAC_CRITICAL_FREE_GB"
    -e "FISH_STREAM_EMPTY_CACHE=$FISH_STREAM_EMPTY_CACHE"
    -e "FISH_LLM_CRITICAL_FREE_GB=$FISH_LLM_CRITICAL_FREE_GB"
    -e "FISH_SESSION_IDLE_MIN_FREE_GB=$FISH_SESSION_IDLE_MIN_FREE_GB"
    -e "FISH_SESSION_IDLE_FORCE_HEAVY_BELOW_GB=$FISH_SESSION_IDLE_FORCE_HEAVY_BELOW_GB"
    -e "FISH_LIGHT_CUDA_CLEANUP_ON_SESSION_IDLE=$FISH_LIGHT_CUDA_CLEANUP_ON_SESSION_IDLE"
    -e "FISH_WARMUP_STREAMING=$FISH_WARMUP_STREAMING"
    -e "FISH_STREAMING_EAGER_DECODE=$FISH_STREAMING_EAGER_DECODE"
...
if [[ "$START_SESSION" == "1" ]]; then
  ...
  FISH_SESSION_CLEANUP_ON_CLOSE="$FISH_SESSION_CLEANUP_ON_CLOSE" \
  WAIT_FOR_MODEL=1 \
