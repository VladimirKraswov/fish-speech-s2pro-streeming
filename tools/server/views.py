...
def _is_no_audio_generated_error(exc: Exception | str | None) -> bool:
    if exc is None:
        return False
...
def _collect_non_streaming_audio(engine, req) -> np.ndarray:
    ...
    Если есть final с полным буфером — используем его.
    Иначе собираем segment-части.
    """
    segments = []
    final_audio = None
    ...
    try:
        ...
        if req.control == "cleanup":
            try:
                ...
                logger.error(f"Error in TTS cleanup: {e}", exc_info=True)
                raise HTTPException(
                    HTTPStatus.INTERNAL_SERVER_ERROR,
